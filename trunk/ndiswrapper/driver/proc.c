...
static struct proc_dir_entry *ndis_proc_entry, *ndis_proc_entry_wep;
...
int ndis_init_proc(struct ndis_handle *handle)
{
	...
	ndis_proc_entry = proc_mkdir(dev->name, NULL);
	...
	if (create_proc_read_entry("stats", 0, ndis_proc_entry,
				   ndis_proc_read_stats, handle) == NULL)
		printk(KERN_INFO "%s: Couldn't create proc entry for 'stats'\n", dev->name);
	ndis_proc_entry_wep = create_proc_entry("wep", 0, ndis_proc_entry);
