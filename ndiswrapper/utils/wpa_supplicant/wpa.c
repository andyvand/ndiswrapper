...
{
	char *title = "PMK Name";
	unsigned char *addr[3] = { title, aa, spa };
	unsigned int len[3] = { 8, ETH_ALEN, ETH_ALEN };
...
static void pmksa_cache_add(struct wpa_supplicant *wpa_s, u8 *pmk, u8 *aa,
			    u8 *spa)
{
			   ...
			   u8 *nonce1, u8 *nonce2, u8 *ptk, size_t ptk_len)
{
	...
	wpa_hexdump(MSG_DEBUG, "WPA: PMK", pmk, 32);
					  ...
					  struct wpa_eapol_key *key, int ver)
{
	...
	if (wpa_s->proto == WPA_PROTO_RSN) {
		...
		wpa_hexdump(MSG_DEBUG, "RSN: msg 1/4 key data",
			    key->key_data, be_to_host16(key->key_data_length));
		while (pos + 1 < end) {
			if (pos + 2 + pos[1] > end) {
	...
	if (pmkid && wpa_s->cur_pmksa &&
		...
		if (eapol_sm_get_key(wpa_s->eapol, wpa_s->pmk, PMK_LEN) == 0) {
			wpa_hexdump(MSG_DEBUG, "WPA: PMK from EAPOL state "
				    "machines", wpa_s->pmk, PMK_LEN);
			pmksa_cache_add(wpa_s, wpa_s->pmk, src_addr,
					wpa_s->own_addr);
		} else {
					  ...
					  int extra_len, int ver)
{
	...
	if (gtk) {
		...
		case WPA_CIPHER_WEP104:
			if (gtk_len != 13) {
				wpa_printf(MSG_WARNING, "WPA: Unsupported "
					   "WEP104 Group Cipher key length "
					   "%d.", gtk_len);
				return;
			}
			alg = WPA_ALG_WEP;
		...
		case WPA_CIPHER_WEP40:
			if (gtk_len != 5) {
				wpa_printf(MSG_WARNING, "WPA: Unsupported "
					   "WEP40 Group Cipher key length %d.",
					   gtk_len);
				return;
			}
			alg = WPA_ALG_WEP;
			     ...
			     unsigned char *buf, size_t len)
{
	...
	if (memcmp(wpa_s->bssid, "\x00\x00\x00\x00\x00\x00", ETH_ALEN) == 0) {
		/* TODO: could use get_bssid first to verify whether the driver
		 * had just completed association and the EAPOL frame was
		 * processed before the event.. In addition, this could detect
		 * BSSID changes if driver does not report them with wireless
		 * events. */
		wpa_printf(MSG_WARNING, "EAPOL frame received in disassociated"
			   " state - dropped");
		return;
	}
...
{
	static const u8 *dummy = "\x00\x00\x00\x00";
...
int wpa_get_mib(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	if (wpa_s->cur_pmksa) {
		for (i = 0; i < PMKID_LEN; i++) {
			sprintf(pmkid_txt, "%02x", wpa_s->cur_pmksa->pmkid[i]);
		}
	} else
		pmkid_txt[0] = '\0';
				 ...
				 void *ctx)
{
	...
	if (success) {
		if (eapol_sm_get_key(eapol, pmk, PMK_LEN) == 0) {
			wpa_hexdump(MSG_DEBUG, "RSN: PMK from pre-auth",
				    pmk, PMK_LEN);
			pmksa_cache_add(wpa_s, pmk, wpa_s->preauth_bssid,
					wpa_s->own_addr);
		} else {
...
int rsn_preauth_init(struct wpa_supplicant *wpa_s, u8 *dst)
{
	...
	wpa_s->l2_preauth = l2_packet_init(wpa_s->ifname, ETH_P_RSN_PREAUTH,
...
static void rsn_preauth_candidate_process(struct wpa_supplicant *wpa_s)
{
	...
	while (wpa_s->pmksa_candidates) {
		...
		if (memcmp(wpa_s->bssid, candidate->bssid, ETH_ALEN) != 0 &&
		    !pmksa_cache_get(wpa_s, candidate->bssid, NULL)) {
			      ...
			      struct wpa_scan_result *results, int count)
{
	...
	for (i = count - 1; i >= 0; i--) {
		...
		if (r->ssid_len == wpa_s->current_ssid->ssid_len &&
			...
			memcpy(candidate->bssid, r->bssid, ETH_ALEN);
			candidate->next = wpa_s->pmksa_candidates;
			wpa_s->pmksa_candidates = candidate;
			wpa_msg(wpa_s, MSG_DEBUG, "RSN: added PMKSA cache "
				"candidate " MACSTR, MAC2STR(r->bssid));
		}
	}
